...
class TestMusicWindowTreeview(unittest.TestCase):
    ...
    def test_on_song_double_click_no_selection(self, mock_downloader, mock_pygame):
        ...
        try:
            ...
            # 沒有選擇任何項目
            window.song_tree.selection_set([])

            # 呼叫事件處理器,應該不會做任何事
            window._on_song_double_click(event)
    ...
    def test_on_song_double_click_plays_song(self, mock_downloader, mock_pygame):
        ...
        try:
            ...
            # Mock _play_song 方法
            window._play_song = Mock()

            # 選擇第二首歌(索引 1)
            children = window.song_tree.get_children()
            window.song_tree.selection_set(children[1])
            ...
            # 確認 current_index 被設定為 1
            self.assertEqual(window.current_index, 1)

            # 確認 _play_song 被呼叫並傳入第二首歌
    ...
    def test_on_song_double_click_first_song(self, mock_downloader, mock_pygame):
        ...
        try:
            ...
            # 選擇第一首歌
            children = window.song_tree.get_children()
            window.song_tree.selection_set(children[0])
    ...
    def test_on_song_double_click_last_song(self, mock_downloader, mock_pygame):
        ...
        try:
            ...
            # 選擇最後一首歌
            children = window.song_tree.get_children()
            window.song_tree.selection_set(children[2])
