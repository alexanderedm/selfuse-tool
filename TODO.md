...
- [ ] **複雜度優化 - 第二輪** (預估: 3-5 天)
  ...
  - [ ] 重構剩餘 7 個 CC > 10 的函數:
    ...
    3. [ ] music_library_view.py _on_category_select_internal (CC 11 → 6)
    4. [ ] music_metadata_fetcher.py fetch_metadata (CC 10 → 6)
    5. [ ] music_metadata_fetcher.py update_song_metadata (CC 10 → 6)
    6. [ ] stats_window.py show (CC 10 → 6)
    7. [ ] rss_manager.py is_valid_rss_url (CC 9 → 5)
  - [ ] 消除重複程式碼 (DRY 原則)
  - [ ] 統一錯誤處理模式
  - **目標**: xenon 零違規，所有函數 CC < 10
  - **當前進度**: 2/9 完成 (22%)，xenon 違規 5 → 4，CC > 10 函數 9 → 8
